...
const checkEventSuccessInBatch = (event: SubstrateEvent): boolean => {
    ...
    const batchInterruptedIndex = extrinsicEvents.findIndex((item) => item.event.method === 'BatchInterrupted' && item.event.section === 'utility')
...
const checkEventSuccessInBatchAll = (event: SubstrateEvent): boolean => {
    const extrinsic = event.extrinsic
    const extrinsicEvents = extrinsic.events
    const isSuccess = extrinsicEvents.findIndex((item) => item.event.method === 'ExtrinsicSuccess' && item.event.section === 'system') !== -1
    const isBatchCompleted = extrinsicEvents.findIndex((item) => item.event.method === 'BatchCompleted' && item.event.section === 'utility') !== -1
...
const isEventSuccess = (event: SubstrateEvent): boolean => {
    if (event.extrinsic){
        ...
        if (isBatch){
            ...
            return extrinsicEvents.findIndex((item) => item.event.method === 'ExtrinsicSuccess' && item.event.section === 'system') !== -1
        }
    }else{
